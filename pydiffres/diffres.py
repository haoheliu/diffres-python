...
class DiffRes(Base):
    def __init__(self, in_t_dim, in_f_dim, dimension_reduction_rate, learn_pos_emb=False):
        ...
        self.model = DilatedConv(in_channels=self.input_f_dim, dilation_rate=1, input_size=self.input_seq_length, kernel_size=5, stride=1)

    def forward(self, x):
        ret = {}
        score = torch.sigmoid(self.model(x.permute(0,2,1)).permute(0,2,1))
        score, _ = self.score_norm(score, self.output_seq_length)
        mean_feature, max_pool_feature, mean_pos_enc = self.frame_warping(x.exp(), score, total_length=self.output_seq_length)
        ...
        ret['guide_loss'], ret['activeness'] = self.guide_loss(x, importance_score=score)
        return ret
    ...
    def visualize(self, ret, savepath="."):
        ...
        for i in range(x.size(0)):
            ...
            plt.subplot(413)
            plt.title("DiffRes mel-spectrogram (Using avgpool frame aggregation function)")
