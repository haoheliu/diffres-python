...
class BaseT(nn.Module):
    def __init__(
        ...
        super(BaseT, self).__init__()

        assert dimension_reduction_rate < 1.0, (
            "Error: Invalid dimension reduction rate %s" % dimension_reduction_rate
        )

        self.dimension_reduction_rate = dimension_reduction_rate
        self.input_seq_length = in_t_dim
        self.input_f_dim = in_f_dim
        self.output_seq_length = int(
            self.input_seq_length * (1 - self.dimension_reduction_rate)
        )
...
class BaseF(nn.Module):
    def __init__(
        ...
        assert in_f_dim % 2 == 0, (
            "Error: The frequency dimension should be a even number, got %s" % in_f_dim
        )
        assert dimension_reduction_rate < 1.0, (
            "Error: Invalid dimension reduction rate %s" % dimension_reduction_rate
        )
